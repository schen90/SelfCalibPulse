//./macros/MakeDataG4_noPS_22Na G4inputfile outputfile
...
double threshold2 = 200; // threshold 200 keV for core energy (trigger)
...
void MakeDataG4_noPS_22Na(string G4inputfile, string outputfile, int mult = 1){
  ...
  while(!feof(fp)){ // loop events in G4inputfile
    ...
    sscanf(buffer,"%d",&iff);
    if(iff<-99) continue; //skip -100, -101, -102
    ...
    if(counter>mult){ readflag=1; counter=1;}
...
int main(int argc, char *argv[]){
  ...
  if(argc>3){
    MakeDataG4_noPS_22Na( string(argv[1]), string(argv[2]), atoi(argv[3]) );
  }else if(argc>2){
    MakeDataG4_noPS_22Na( string(argv[1]), string(argv[2]), 1 );
  }else if(argc>1){
    MakeDataG4_noPS_22Na( string(argv[1]), "rootfiles/noPS/G4SimData0000.root", 1 );
  }else{
    MakeDataG4_noPS_22Na( "trunk/GammaEvents.0000", "rootfiles/noPS/G4SimData0000.root", 1 );
  }
