...
void testFFT(){
  ...
  for(ievt=0; ievt<nevts; ievt++){
    ...
    // add noise
    //for(int isig=0; isig<NSig; isig++) h->SetBinContent(isig,1); //uniform
    for(int isig=0; isig<NSig; isig++) h->SetBinContent(isig,sin(10*2*TMath::Pi()/NSig*isig)); //sin
    //for(int isig=0; isig<NSig; isig++) h->SetBinContent(isig,TMath::Gaus(isig,NSig/2,3)); // spark
