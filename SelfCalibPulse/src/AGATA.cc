...
void AGATA::LoadHCfiles(int detid){
  ...
  for(ihc=0; ihc<Nhcs; ihc++){
    ...
    HitCollection* ahc = new HitCollection(det, seg, index, calpos);
...
void AGATA::LoadEvtHitsfiles(int iconfig){
  ...
  for(; atomrun<fConfigs[iconfig].MaxRun+1;){ // loop runs 
	...
	Hit *ahit = new Hit(detid, segid, depE, initpos);
...
int AGATA::InitPSCandHC(int detid, int segid){
  ...
  if(freeHCs[detid][segid].size()>0){ // use old HCs
    ...
    HitCollection* newhc = new HitCollection(detid, segid, ipsc, dummycalpos);
...
void AGATA::FindInitZone(PS *aps, vector<int> *initzone){
  ...
  // 0:iseg and 1:core
  copy_n( aps->opulse[fseg[0]], LOOP_SAMP, apulse);
  copy_n( aps->opulse[fseg[1]], LOOP_SAMP, bpulse);
  ...
  // neighbor sector 2 & 3
  copy_n( aps->opulse[fseg[2]], LOOP_SAMP, apulse);
  copy_n( aps->opulse[fseg[3]], LOOP_SAMP, bpulse);
...
float AGATA::FindMaxDev(PS *aps, Hit *ahit){
  ...
  for(HitCollection* ahc : *hcs){
    ...
    for(int is=0; is<NCOMP; is++){
      ...
      copy_n(apsc->spulse[iseg], LOOP_SAMP, bsegpulse);
