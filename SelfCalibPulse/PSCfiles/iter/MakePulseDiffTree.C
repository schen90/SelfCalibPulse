...
// calib pscfile
string pscfile = "it0/Det0000_fit4.root";
...
void MakePulseDiffTree(){
  ...
  float dist;  
  float diffr, diffz, rdiffphi, diffphi;
  int dbsegi;
  int ngrid, extrpl;  
  float chi2;

  TFile *fout = new TFile("PulseDiffTree.root","RECREATE");
  ...
  outtree->Branch("phi0",&phi0);
  outtree->Branch("r0",&r0);
  outtree->Branch("z0",&z0);

  outtree->Branch("phi1",&phi1);
  outtree->Branch("r1",&r1);
  outtree->Branch("z1",&z1);

  outtree->Branch("dist",&dist);
  outtree->Branch("diffr",&diffr);
  outtree->Branch("diffz",&diffz);
  outtree->Branch("rdiffphi",&rdiffphi);

  outtree->Branch("dbseg",&dbsegi);
  outtree->Branch("ngrid",&ngrid);
  outtree->Branch("extrpl",&extrpl);
  ...
  TProfile2D *pxy[6];
  TProfile2D *pxz = new TProfile2D("pxz","chi2",nbinsxy,xymin,xymax,nbinsz,zmin,zmax);
  TProfile3D *pxyz = new TProfile3D("pxyz","chi2",nbinsxy,xymin,xymax,nbinsxy,xymin,xymax,nbinsz,zmin,zmax);
  TProfile3D *pxyz2 = new TProfile3D("pxyz2","chi2",nbinsxy,xymin,xymax,nbinsxy,xymin,xymax,nbinsz,zmin,zmax);
  for(int isli=0; isli<6; isli++){
    pxy[isli] = new TProfile2D(Form("pxy%d",isli),Form("chi2 {slice%d}",isli),nbinsxy,xymin,xymax,nbinsxy,xymin,xymax);
  }
  ...
  for(ientry=0; ientry<nentries; ientry++){
    ...
    if(diffphi<-180) diffphi+=360;
    rdiffphi = (r1+r0)/2*diffphi/180*TMath::Pi();
    ...
    if(seg==dbsegi && ngrid==8 && npaths>150){
      //if(seg==dbsegi && ngrid==8 && npaths>30 && chi2<10){
      //if(seg==dbsegi && ngrid==8 && ((extrpl==0&&npaths>100)||(extrpl>0&&npaths>150)) ){
      int isli = seg%6;
      pxy[isli]->Fill(cadpos2[0],cadpos2[1],chi2);
      pxz->Fill(cadpos2[0],cadpos2[2],chi2);

      if(chi2>20) pxyz->Fill(cadpos2[0],cadpos2[1],cadpos2[2],20);
      else        pxyz->Fill(cadpos2[0],cadpos2[1],cadpos2[2],chi2);

      if(chi2>10){
	pxyz2->Fill(cadpos2[0],cadpos2[1],cadpos2[2],chi2);
      }
    }
    
  }
  cout<<"\r finish "<<ientry<<" / "<<nentries<<"..."<<endl;

  fout->cd();
  outtree->Write();
  for(int isli=0; isli<6; isli++) pxy[isli]->Write();
  pxz->Write();
  pxyz->Write();
  pxyz2->Write();
  fout->Close();
