...
// main
void MakePSBaseRealPS(string DBPosfile, string PSCfile, string Basefile){
  ...
  for(ientry=0; ientry<nentries; ientry++){
    ...
    */
    ...
    for(int iseg=0; iseg<nsegcore; iseg++)
      for(int isig=0; isig<nsig; isig++)
	tmppulse(iseg*nsig+isig,0) = realspulse(iseg*nsig+isig,0);
...
int main(int argc, char *argv[]){
  if(argc>3){
    MakePSBaseRealPS(string(argv[1]), string(argv[2]), string(argv[3]));
  }else{
    MakePSBaseRealPS("G4Sim/pulsedb/pulseA.root", "PSCfiles/runMix/gp_mh4/it2/Det0000_fit4.root", "PSBase/Det0000.root");
  }
  return 0;
}
#endif
